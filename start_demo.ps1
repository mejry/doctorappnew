...
foreach ($service in $services) {
    ...
    if ($service -eq "api-gateway") {
        Start-Process "node" -ArgumentList "server.js" -WorkingDirectory "$basePath\services\$service" -WindowStyle Normal
    } else {
        Start-Process "npm" -ArgumentList "run dev" -WorkingDirectory "$basePath\services\$service" -WindowStyle Normal
    }
...
Start-Process "flutter" -ArgumentList "run -d chrome --web-port 3000" -WorkingDirectory "$basePath\frontend" -WindowStyle Normal
