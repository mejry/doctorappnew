...
const hasPermission = (requiredPermission) => (req, res, next) => {
  try {
    ...
    logger.info(`Checking permission "${requiredPermission}" for user ${req.user.id} with role ${req.user.role}`);
    ...
    if (!req.user.permissions.includes(requiredPermission)) {
      logger.error(`Permission denied: User ${req.user.id} (${req.user.role}) does not have permission: ${requiredPermission}`);
    ...
    logger.info(`Permission granted: User ${req.user.id} has permission: ${requiredPermission}`);
    next();
  } catch (error) {
...
const hasAnyPermission = (requiredPermissions) => (req, res, next) => {
  try {
    ...
    if (!hasPermission) {
      logger.error(`Permission denied: User ${req.user.id} lacks any of: ${requiredPermissions.join(', ')}`);
      return res.status(403).json({
        success: false,
        message: 'Permission denied',
