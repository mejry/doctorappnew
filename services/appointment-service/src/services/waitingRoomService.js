...
class WaitingRoomService {
  ...
  async getTodaysQueue(doctorId) {
    try {
      ...
      const queue = await WaitingRoomEntry.find({
        ...
        .populate(
          "appointmentId",
          "patientName type time duration patientContact",
        )
        .sort({
  ...
  async markNoShow(appointmentId, markedBy, reason) {
    try {
      const waitingRoomEntry = await WaitingRoomEntry.findOne({
        appointmentId,
      });
    ...
    try {
      ...
      if (waitingRoomEntry.consultationStartedAt) {
        const duration = Math.floor(
          (completionTime - waitingRoomEntry.consultationStartedAt) /
            (1000 * 60),
        );
          ...
          try {
            const notificationResult =
              await notificationService.sendDelayNotification({
                ...
                delayMinutes,
                reason,
                estimatedNewTime: this.calculateNewTime(
                  appointment.time,
                  delayMinutes,
                ),
              });
            ...
            logger.warn(
              `Failed to send notification to ${appointment.patientName}:`,
              notifError,
            );
            notifications.push({
              appointmentId: appointment._id,
