...
const verifyToken = (req, res, next) => {
  try {
    ...
    console.log(`👤 User authenticated: ID=${userId}, Role=${userRole}, Permissions=[${userPermissions.join(', ')}]`);
    next();
  } catch (error) {
...
const isStaff = (req, res, next) => {
  ...
  if (!staffRoles.includes(req.user.role)) {
    console.warn(`⚠️ Non-staff access attempt by user ${req.user.id} with role ${req.user.role}`);
